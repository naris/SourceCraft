...
my ($filename);
$filename = 'sidewinder-' . $version;
if ($^O eq "linux")
{
    $filename .= '.tar.gz';
    print "tar zcvf $filename sourcemod\n";
    system("tar zcvf $filename sourcemod");
}
else
{
    $filename .= '.zip';
    print "zip -r $filename sourcemod\n";
    system("zip -r $filename sourcemod");
}

my ($major,$minor) = ($version =~ /^(\d+)\.(\d+)/);
$ftp_path .= "/sidewinder/$major.$minor";
