...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	{
#if defined SOURCECRAFT
		thisRaceID=CreateRace("shadow_hunter", .faction=OrcishHorde, .type=Biological);
#else
		...
		SetUpgradeCooldown(thisRaceID, ULT_VOODOO, 20.0); // Can be altered in the race config file
...
#if defined SOURCECRAFT
AttachDualParticle(ent, String:particleType[], controlpoint)
#else
...
StopParticleEffect(client, bKill)
{
	if(War3_GetGame() == Game_TF)
	{
		for(new i=1; i <= MaxClients; i++)
		{
			if(ParticleEffect[client][i] != 0 && IsValidEdict(ParticleEffect[client][i]))
			{
				decl String:className[64];
				if (GetEdictClassname(ParticleEffect[client][i], className, sizeof(className)))
				{
			...
			if(ParticleEffect[i][client] != 0 && IsValidEdict(ParticleEffect[i][client]))
			{
				decl String:className2[64];
				if (GetEdictClassname(ParticleEffect[i][client], className2, sizeof(className2)))
				{
									...
									if(GetVectorDistance(HealerPos, TeammatePos) <= maxDistance)
									{
										if(StrEqual(className, "info_particle_system"))
											...
											switch(GetClientTeam(client))
											{
												case(2):
#if defined SOURCECRAFT
													AttachDualParticle(client, "medicgun_beam_red", i);
#else
													AttachParticle(client, "medicgun_beam_red", i);
#endif
												case(3):
#if defined SOURCECRAFT
													AttachDualParticle(client, "medicgun_beam_blue", i);
#else
