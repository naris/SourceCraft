...
{
#if defined SOURCECRAFT
	thisRaceID=CreateRace("farseer", .name="Far Seer", .faction=OrcishHorde, .type=Biological);
#else
	...
	SetUpgradeCooldown(thisRaceID, SKILL_FAR_SEER, 10.0); // Can be altered in the race config file
	SetUpgradeEnergy(thisRaceID, SKILL_FAR_SEER, GetUpgradeCooldown(thisRaceID,SKILL_FAR_SEER));

	SetUpgradeCooldown(thisRaceID, SKILL_FERAL, 15.0); // Can be altered in the race config file
