...
{	
	...
	if(num==230)
	{
		...
		thisRaceID=CreateRace("rainbowdash", .name="[MLP:FIM] Rainbow Dash", .faction=Pony, .type=Biological);
#else
		...
		SetUpgradeCooldown(thisRaceID, SKILL_SPEED, 20.0); // Can be altered in the race config file
		...
		GetConfigArray("chance",  EvadeChance, sizeof(EvadeChance),
			 	EvadeChance, thisRaceID, SKILL_EVADE);
		SetArrayArray(evasiondata,0,EvadeChance,sizeof(EvadeChance));

		GetConfigFloatArray("attack_speed",  attackspeed, sizeof(attackspeed),
					attackspeed, thisRaceID, SKILL_SWIFT);
