...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	{
#if defined SOURCECRAFT
		thisRaceID=CreateRace("mage", .faction=HumanAlliance, .type=Biological);
#else
		...
		SetUpgradeCooldown(thisRaceID, ULT_FLAMESTRIKE, 20.0); // Can be altered in the race config file
		...
		RevivalDelay=GetConfigFloat("percent", RevivalDelay, thisRaceID, ULT_FLAMESTRIKE);
...
public RoundStartEvent(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	for(new i=1;i<=MaxClients;i++)
	{
		...
		if(ValidPlayer(i) && skill_level_revive > 0)
		{
...
#if defined SOURCECRAFT
								CreateTimer(RevivalDelay,DoRevival,victim);
#else
