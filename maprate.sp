...
public Menu_Rate(Handle:menu, MenuAction:action, param1, param2) {
	...
	case MenuAction_Select: {
	    ...
	    if (g_SQLite) {
		Format(query, sizeof(query), "REPLACE INTO %s VALUES ('%s', '%s', %d, DATETIME('NOW'))",
		    g_table_name,
		    steamid,
	    	    map, 
	    	    rating
		...
		Format(query, sizeof(query), "INSERT INTO %s SET map = '%s', steamid = '%s', rating = %d, rated = NOW() ON DUPLICATE KEY UPDATE rating = %d, rated = NOW()",
		    g_table_name,
	    	    map, 
	    	    steamid,
	    	    rating,
