...
public OnPluginStart()
{
    ...
    g_IsAmmopacksOn = CreateConVar("sm_ammopacks","3","Enable/Disable ammopacks (0 = disabled | 1 = on death | 2 = on command | 3 = on death and command)");
    g_AmmopacksSmall = CreateConVar("sm_ammopacks_small","50","Metal required for small Ammopacks");
...
{	
    ...
    for (new i = 1; i <= maxclients; i++)
    {
        ...
        if (!g_IsButtonDown[i] && IsClientInGame(i) && TF2_GetPlayerClass(i) == TFClass_Engineer)
        {
...
public ConVarChange_AmmopacksMetal(Handle:convar, const String:oldValue[], const String:newValue[])
{
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    {
        case 1:
            PrintToChatAll("[SM] %t", "OnDeath Ammopacks");
        case 2:
            PrintToChatAll("[SM] %t", "OnCommand Ammopacks");
        case 3:
            PrintToChatAll("[SM] %t", "OnDeathAndCommand Ammopacks");
    }
}
...
stock bool:TF_DropAmmopack(client, bool:cmd)
{
    ...
    new metal = (cmd) ? TF2_GetMetalAmount(client) : g_EngiMetal[client];
    if (metal >= AmmopacksFull && AmmopacksFull != 0)
    {
        if (cmd)
            TF2_SetMetalAmount(client, (metal-AmmopacksFull));
    ...
    {
        if (cmd)
            TF2_SetMetalAmount(client, (metal-AmmopacksMedium));
    ...
    {
        if (cmd)
            TF2_SetMetalAmount(client, (metal-AmmopacksSmall));
