...
new String:cfgfile[PLATFORM_MAX_PATH];
...
public OnAutoConfigsBuffered(){
	...
	if(strlen(currentMap) > 2 && !(StrContains(currentMap,breaker) == -1)){
		...
		BuildPath(Path_SM,cfgfile,sizeof(cfgfile),"../../cfg/%s.cfg", currentMap);
		...
		if(debugmsg == true){
			LogMessage("0 Patch cfgfile: %s",cfgfile);
		...
		if(FileExists(cfgfile)){
			readFileConfig(cfgfile);
		} else if(debugmsg == true){
			LogMessage("0 Mapfile dosent exists: %s",cfgfile);
		}
		...
		BuildPath(Path_SM,mapfile,sizeof(mapfile),"configs/mapscfg/%s.cfg", currentMap);
		if(debugmsg == true){
			LogMessage("1 Patch cfgfile: %s",cfgfile);
			LogMessage("1 Patch mapfile: %s",mapfile);
		}
		if(FileExists(cfgfile)){
			readFileConfig(cfgfile);
		} else if(debugmsg == true){
			LogMessage("1 Mapfile dosent exists: %s",cfgfile);
		}
		if(FileExists(mapfile)){
			readFileConfig(mapfile);
		} else if(debugmsg == true){
