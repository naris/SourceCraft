...
public Plugin:myinfo = {
	name = "The Amplifier",
	author = "Plugin: Dr.Eggman; Idea & Model: Jumento M.D.; Great Help: -=|JFH|=-Naris",
...
public OnPluginStart()
{
	...
	cvarCondition = CreateConVar("amplifier_condition", "11", "Condition that The amplifier dispenses (11=full crits, 16=mini crits, etc...).", FCVAR_PLUGIN);
					...
					if ((amp>0) && (StrContains(modelname,"plifier")>-1))	//Force off AMP's effect on Dispenser and Rep.Node
					{
						...
						if (AmplifierOn[amp] && !AmplifierSapped[amp])
						{
							...
							if ((TF2_GetPlayerClass(client) == TFClass_Spy) &&
							  TF2_IsPlayerDisguised(client) && !(GetEntProp(client, Prop_Send, "m_nPlayerCond") & 16))
								...
								(!WallBlock || TraceTargetIndex(amp, client, AmplifierPos, Pos)))
							{
								...
								else if ((Condition == TFCond_Kritzkrieged) && MiniCritToSG && (TF2_GetPlayerClass(client) == TFClass_Engineer) && StrEqual(weapon, "tf_weapon_laser_pointer"))
