...
public Handle_Category(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength)
{
	switch (action)
	{
		...
		case TopMenuAction_SelectOption:
		{
			...
			new style = (g_bMvMMode) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT;
			...
			{
				AddMenuItem(hScrambleOptionsMenu, "0", "Start a Scramble", style);
			}
			if (CheckCommandAccess(param, "sm_scramblevote", ADMFLAG_BAN))
			{
				AddMenuItem(hScrambleOptionsMenu, "1", "Start a Vote", style);
			...
			{
				AddMenuItem(hScrambleOptionsMenu, "3", "Force Team Balance", style);
			}
