...
* 		1.4.2 - Naris put distance check back in and added option to log missing weapons to the standard error log.
*/
...
/*
* Uncomment to calculate distance,
* uncomment the next line to use metric units
*/
#define DISTANCE
//#define METRIC
...
/*
* Uncomment to enable logging of missing weapon translations to the standard error log.
*/
#define LOG_MISSING_WEAPONS
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	decl String:szKillType[32], String:szWeapon[64], String:szTrans[32], String:szDistance[32];
	...
	*/
	new bool:bWeaponFound = TranslationExists(szWeapon);
	#if defined LOG_WEAPON_ERRORS
	if (!bWeaponFound)
		LogToFileEx(g_szLogFile, "Translation missing for weapon: %s", szWeapon);
	#elseif defined LOG_MISSING_WEAPONS
	if (!bWeaponFound)
		LogError("Translation missing for weapon: %s", szWeapon);
	#endif	
	...
	*/
	...
	{
		if (bWeaponFound)
			strcopy(szTrans, sizeof(szTrans), "died before killing");
		else
			strcopy(szTrans, sizeof(szTrans), "died before killing nowpn");
	...
	*/
	...
	{
		if (TF2_IsPlayerInCondition(iAttacker, TFCond_Buffed))			//Buff Banner
			FormatEx(szKillType, sizeof(szKillType), "minicrit banner");
		else if (TF2_IsPlayerInCondition(iVictim, TFCond_Jarated))		//Jarate
			FormatEx(szKillType, sizeof(szKillType), "minicrit jarate");
		else if (TF2_IsPlayerInCondition(iAttacker, TFCond_CritCola))		//Crit-a-Cola
	...
	*/
	...
	if (bWeaponFound)
		strcopy(szTrans, sizeof(szTrans), "killed you");
	else
		strcopy(szTrans, sizeof(szTrans), "killed you nowpn");
	
	#if defined DISTANCE
		new Float:victimLoc[3], Float:attackerLoc[3], Float:distance;
		GetClientAbsOrigin(iVictim,victimLoc);
		GetClientAbsOrigin(iAttacker,attackerLoc);
		distance = FloatDiv(GetVectorDistance(victimLoc,attackerLoc), 32.0);

		#if defined METRIC
			static const String:units[] = "m";
			distance *= 0.305; //1ft = .305 meters
		#else
			static const String:units[] = "ft";
		#endif
		Format(szDistance, sizeof(szDistance), " @ %0.1f%s", distance, units);
	#else
		szDistance[0] = '\0';
	#endif

	#if defined USECOLORS
	CPrintToChatEx(iVictim, iAttacker, "%t", szTrans, iAttacker, szKillType, szWeapon, GetClientHealth(iAttacker), szDistance);
	#else
	PrintToChat(iVictim, "%t", szTrans, iAttacker, szKillType, szWeapon, GetClientHealth(iAttacker), szDistance);
	#endif
