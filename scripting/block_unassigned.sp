...
public DoClientSentence(client) {
	...
	// PRIVATELY WARN
	if (sentence == 1) {
		LogMessage("%s was warned for exploiting the unassigned team", name);
		...
		if (OtherAlertTimers[client] == INVALID_HANDLE) {
			...
			LogMessage("%N was warned for exploiting the unassigned team", client);
		}
	} else if (sentence == 3) {
		LogMessage("%s was kicked for exploiting the unassigned team", name);
		PrintToChatAll("%s was kicked for exploiting the unassigned team", name);
		KickClient(client, BU_WARNING_MSG);
	} else if (sentence == 4) {
		LogMessage("%s was BANNED for exploiting the unassigned team", name);
		PrintToChatAll("%s was BANNED for exploiting the unassigned team", name);
		...
		LogMessage("%s was moved to spectator for exploiting the unassigned team", name);
	}
	else {
		// Do nothing
	}
...
public OnPluginStart() {
	...
	cvar_Ban_Time = CreateConVar("sm_bu_ban_time", "0", "If action is at 4, ban for X minutes");
	cvar_Action = CreateConVar("sm_bu_action", "3", "Action to take on that user (0-5). Default: Nothing.");
