...
public Event_PlayerChangename(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!IsRenameNeeded(client,newName))
	{
 ...
 */
IsRenameNeeded(client, String:newName[255])
{
	...
	do
	{
		...
		if (StrContains(newName, section, false) != -1)
		{
			KvGetString(g_kvNames, "isPartial", isPartial, sizeof(isPartial));

			if (StrEqual(isPartial, "1") || StrEqual(newName, section, false))
			{
	...
	{
		LogAction(0, client, "Reserved name (%s) found, checking authorization for user %s", newName, steamId);
