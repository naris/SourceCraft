...
#define CAN_GRAB_BUILDINGS		    (1 << 2)
#define CAN_GRAB_OTHER_BUILDINGS    (1 << 3)
#define CAN_THROW_BUILDINGS         (1 << 4)
...
new bool:gPickupHooked = false;

// forwards
new Handle:fwdOnPickup;
...
public bool:AskPluginLoad(Handle:myself,bool:late,String:error[],err_max)
{
    ...
    CreateNative("HookPickup",Native_HookPickup);
    ...
    // Register Forwards
    //fwdOnPickup=CreateGlobalForward("OnPickup",ET_Hook,Param_Cell,Param_Cell,Param_Cell);
    fwdOnPickup=CreateForward(ET_Hook,Param_Cell,Param_Cell,Param_Cell);
...
{  
    ...
    if (grab)
    {
        ...
        if (gPickupHooked && builder)
        {
            Call_StartForward(fwdOnPickup);
...
public Native_HookPickup(Handle:plugin,numParams)
{
    if(numParams == 1)
    {
        AddToForward(fwdOnPickup, plugin, Function:GetNativeCell(1));
        gPickupHooked = true;
    }
}
