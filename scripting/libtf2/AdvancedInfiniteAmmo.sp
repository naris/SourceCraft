...
{
	g_bInfiniteAmmo[client] = (!g_bNativeControl && GetConVarBool(g_hAllInfiniteAmmo));
	if(IsValidClient(client, false))
	{
		SDKHook(client, SDKHook_PreThink, SDKHook_OnPreThink);
		SDKHook(client, SDKHook_WeaponEquipPost, SDKHook_OnWeaponEquipPost);
...
bool:GiveInfiniteAmmo(client, iWeapon)
{
...
stock SetAmmo(client, iWeapon, iAmmo = 999)
{
    ...
    if(iAmmoType != -1) SetEntProp(client, Prop_Send, "m_iAmmo", iAmmo, _, iAmmoType);
}
...
{
    SetEntProp(client, Prop_Send, "m_iAmmo", iMetal, _, 3);
}
...
public Native_FillWeaponAmmo(Handle:plugin, numParams)
{
    ...
    if (IsValidWeapon(weapon))
        return GiveInfiniteAmmo(client, weapon);
    else
        return false;
}
...
public Native_FillAmmo(Handle:plugin, numParams)
{
    ...
    {
        return GiveInfiniteAmmo(client, g_iClientWeapons[client][slot]);
    }
