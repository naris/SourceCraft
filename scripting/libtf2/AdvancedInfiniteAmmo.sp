...
#define PLUGIN_VERSION "1.5.3-sc"
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("AIA_Control", Native_AIA_Control);
	...
	CreateNative("AIA_FillWeaponAmmo", Native_AIA_FillWeaponAmmo);
	CreateNative("AIA_FillAmmo", Native_AIA_FillAmmo);
...
public void TF2_OnWaitingForPlayersStart()
{
	...
	if(g_hWaitingForPlayers.BoolValue)
	{
		...
		if(g_bAnnounce && g_hChat.BoolValue) PrintToChatAll("[SM] Waiting For Players Started - Advanced Infinite Ammo enabled");
	}	
}
...
{	
	if(!g_bWaitingForPlayers)
	{
		...
		if(g_bAnnounce && g_hChat.BoolValue) PrintToChatAll("[SM] Round Start - Advanced Infinite Ammo disabled");
	}
}
...
public void Event_RoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	if(!g_hAllInfiniteAmmo.BoolValue && g_hRoundWin.BoolValue)
	{
		...
		if(g_bAnnounce && g_hChat.BoolValue) PrintToChatAll("[SM] Round Win - Advanced Infinite Ammo enabled");
	}
}
...
public Action Command_SetAIA(int client, int args)
{
	...
	switch(args)
	{
		...
		case 0:
		{
			...
			if(g_bAnnounce && g_hChat.BoolValue) ShowActivity2(client, "[SM] ","Advanced Infinite Ammo for %N %s", client, bOnOff ? "enabled":"disabled");			
		...
		case 2:
		{
			...
			if(g_bAnnounce && g_hChat.BoolValue) ShowActivity2(client, "[SM] ","Advanced Infinite Ammo for %s %s", target_name, bOnOff ? "enabled":"disabled");
...
public Action Command_SetAIATimed(int client, int args)
{
	...
	if(g_bAnnounce && g_hChat.BoolValue) ShowActivity2(client, "[SM] ","Advanced Infinite Ammo enabled for %s for %-.2f seconds", target_name, time);
...
public int Native_AIA_Control(Handle plugin, int numParams)
{
...
public int Native_AIA_FillAmmo(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
...
public int Native_AIA_FillWeaponAmmo(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
