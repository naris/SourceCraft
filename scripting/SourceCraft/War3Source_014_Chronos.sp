...
{	
	...
	{
#if defined SOURCECRAFT
		thisRaceID=CreateRace("chronos", .faction=Hellbourne, .type=Mechanical);
#else
		...
		SetUpgradeCooldown(thisRaceID, SKILL_LEAP, 10.0); // Can be altered in the race config file
		SetUpgradeEnergy(thisRaceID, SKILL_LEAP, GetUpgradeCooldown(thisRaceID,SKILL_LEAP));

		SetUpgradeCooldown(thisRaceID, ULT_SPHERE, 20.0); // Can be altered in the race config file
...
public Action:sphereLoop(Handle:h,any:client){
	if(hasSphere[client]&&SphereEndTime[client]>GetGameTime()&&ValidPlayer(client)){
