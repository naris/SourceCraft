...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	{
#if defined SOURCECRAFT
		thisRaceID=CreateRace("lich", .faction=UndeadScourge, .type=Undead);
#else
		...
		SetUpgradeCooldown(thisRaceID, ULT_DEATHDECAY, 30.0); // Can be altered in the race config file
			...
			if(ValidPlayer(i,true)&&GetClientTeam(i)!=team&&!W3HasImmunity(i,Immunity_Skills))
			{
				...
				if(victimdistance<FrostNovaRadius&&FloatAbs(otherVec[2]-FrostNovaOrigin[attacker][2])<50)
				{
					if(FloatAbs(victimdistance-hitRadius)<(FrostNovaRadius/10.0))
					{
						...
						PrintHintText(attacker,"%T","{client} was slowed by frost nova!",attacker,i);
					}
				}
			}
		}
