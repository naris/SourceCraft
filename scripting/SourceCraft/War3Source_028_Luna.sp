...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	{
#if defined SOURCECRAFT
		thisRaceID=CreateRace("luna", .faction=Sentinel, .type=Biological);
#else
		...
		SetUpgradeCooldown(thisRaceID, SKILL_MOONBEAM, 3.0); // Can be altered in the race config file
		SetUpgradeEnergy(thisRaceID, SKILL_MOONBEAM, GetUpgradeCooldown(thisRaceID,SKILL_MOONBEAM));

		SetUpgradeCooldown(thisRaceID, ULT, 25.0); // Can be altered in the race config file
		SetUpgradeEnergy(thisRaceID, ULT, GetUpgradeCooldown(thisRaceID,ULT));

		// Get Configuration Data
		LucentBeamMin=GetConfigNum("min", LucentBeamMin, thisRaceID, SKILL_MOONBEAM);
		LucentBeamMax=GetConfigNum("max", LucentBeamMax, thisRaceID, SKILL_MOONBEAM);
