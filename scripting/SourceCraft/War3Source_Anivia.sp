...
public OnWar3PluginReady(){
	...
	*/
#if defined SOURCECRAFT
		thisRaceID=CreateRace("anivia", .faction=Sentinel, .type=Elemental);
#else
	...
	ULT=War3_AddRaceSkillT(thisRaceID,"Glacialstorm",true,4); 
		...
		SetUpgradeCooldown(thisRaceID, SKILL_1, 8.0); // Can be altered in the race config file
		SetUpgradeEnergy(thisRaceID, SKILL_1, GetUpgradeCooldown(thisRaceID,SKILL_1));

		SetUpgradeCooldown(thisRaceID, SKILL_2, 8.0); // Can be altered in the race config file
		SetUpgradeEnergy(thisRaceID, SKILL_2, GetUpgradeCooldown(thisRaceID,SKILL_2));

		SetUpgradeCooldown(thisRaceID, ULT, 6.0); // Can be altered in the race config file
...
stock SpawnFrozenProp(const Float:Origin[3],const Float:fAngles[3],String:modelName[],iHealth,iTeamNum,Float:fLifetime,bool:bDrawInvisible=false) {
	...
	if(iHealth>0) {
		SetEntProp(PhysicsProp, Prop_Send, "m_iHealth", iHealth);
