...
public OnWar3LoadRaceOrItemOrdered(num)
{
	...
	{						
#if defined SOURCECRAFT
		thisRaceID=CreateRace("crypt", .faction=UndeadScourge, .type=Undead);
#else
		...
		SetUpgradeCooldown(thisRaceID, ULT_LOCUST, 20.0); // Can be altered in the race config file
...
public OnWar3EventPostHurt(victim,attacker,damage)
{
	if(W3GetDamageIsBullet()&&ValidPlayer(victim,true)&&ValidPlayer(attacker,true)&&GetClientTeam(victim)!=GetClientTeam(attacker))
	{
		if(War3_GetRace(victim)==thisRaceID &&W3Chance(W3ChanceModifier(attacker)) )
		{
			...
			skill_level = War3_GetSkillLevel(victim,thisRaceID,SKILL_IMPALE);
