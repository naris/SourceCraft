...
BuildLogFilePath()
{
	...
	if (!StrEqual(AFKM_LogFile, LogFile))
	{
		LogAction(0, -1, "[AFK Manager] Log File: %s", AFKM_LogFile);
		LogToFile(AFKM_LogFile, "AFK Manager Log File");
...
RegisterCvars()
{
	...
	hCvarLocationThreshold = CreateConVar("sm_afk_location_threshold", "30.0", "Threshold for amount of movement required to mark a player as AFK. [0 = NONE, DEFAULT: 30.0]");
...
#endif
			hAFKTimers[index] = CreateTimer(AFK_CHECK_INTERVAL, Timer_CheckPlayer, index, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
...
bool:CheckObserverAFK(client)
{
	...
	else
	{
		...
		else if (g_Last_Spec > 0)
		{
			...
			{
#if _DEBUG > 1
				LogDebug(false, "CheckObserverAFK - Observer looking at new target? Old: %i New: %i Not AFK?", g_Last_Spec, iSpecTarget[client]);
#endif
				// Old target died and is no longer valid.
				if (IsValidClient(g_Last_Spec, false) && !IsPlayerAlive(g_Last_Spec))
				{
#if _DEBUG > 1
					LogDebug(false, "CheckObserverAFK - Observer old target is no longer valid as they are dead? Old: %i", g_Last_Spec);
#endif
					return true;
				}
				else
...
Action:MoveAFKClient(client, bool:Advertise=true)
{
	...
	if (Advertise)
		PrintToChatAll("[AFK Manager] %t", "Move_Announce", f_Name);
	else
		PrintToChat(client, "[AFK Manager] %t", "Move_Announce", f_Name);
...
Action:KickAFKClient(client)
{
	...
	PrintToChatAll("[AFK Manager] %t", "Kick_Announce", f_Name);
