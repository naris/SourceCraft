...
public Plugin:myinfo =
{
	name = "Stunmod",
	author = "DarthNinja",
	description = "Use Trigger:Stun on players anywhere!",
...
public Action:Cmd_Stun(client, args)
{
	...
	for (new i = 0; i < target_count; i ++)
	{
		...
		ShowActivity2(client, "[SM] ","Stunned '%N'.", target_list[i]);
	}
	return Plugin_Handled;
}
...
public Action:Cmd_GhostStun(client, args)
{
	...
	for (new i = 0; i < target_count; i ++)
	{
		TF2_StunPlayer(target_list[i], duration, _, 193, 0);
		LogAction(client, target_list[i], "\"%L\" ghost-stunned \"%L\" ", client, target_list[i]);
		ShowActivity2(client, "[SM] ","Ghost-Stunned '%N'.", target_list[i]);
	}
	return Plugin_Handled;
}
...
public Action:Cmd_Bonk(client, args)
{
	if (args != 1 && args != 2)
	{
		ReplyToCommand(client, "Usage: sm_bonk <target> [duration]");
	...
	for (new i = 0; i < target_count; i ++)
	{
		TF2_StunPlayer(target_list[i], duration, 0.0, TF_STUNFLAGS_BIGBONK, 0);
		LogAction(client, target_list[i], "\"%L\" bonked \"%L\" ", client, target_list[i]);
		ShowActivity2(client, "[SM] ","Bonked '%N'.", target_list[i]);
	}
	return Plugin_Handled;
}
...
public Action:Cmd_StunMe(client, args)
{
	...
	new checkEnable = GetConVarInt(allowSelfstun);
	if (checkEnable == 0)
	{
...
public Action:Cmd_GhostStunMe(client, args)
{
	...
	TF2_StunPlayer(client, duration, _, 193, 0);
