...
public SQL_CheckBan(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    {
        LogError("[MySQL Bans] Lookup failed for <%s>,<%s>,<%s>, error: %s",
                 player_name, ipaddr, steam_id, error);

        /* Error Checking */
        if (check >= 1)
        {
            LogAction(client, -1, "[MySQL Bans] Lookup failed for <%s>,<%s>,<%s>",
                      player_name, ipaddr, steam_id);

            PrintToServer("\x04[MySQL Bans]\x03 Lookup failed for <%s>,<%s>,<%s>",
                          player_name, ipaddr, steam_id);

            if (check >= 2)
            {
                PrintToChatAll("\x04[MySQL Bans]\x03 Lookup failed for <%s>,<%s>,<%s>",
                               player_name, ipaddr, steam_id);
            }
        }
        return;
    ...
    if (!SQL_FetchRow(hndl))
    {
        ...
        if (check >= 1)
        {
            LogAction(client, -1, "[MySQL Bans] <%s>,<%s>,<%s> NOT found in bans database. Play nice now!>",
                      player_name, ipaddr, steam_id);

            PrintToServer("\x04[MySQL Bans]\x03 <%s>,<%s>,<%s> NOT found in bans database. Play nice now!>",
                          player_name, ipaddr, steam_id);

            if (check >= 2)
                PrintToChatAll("\x04[MySQL Bans]\x03 <%s>,<%s>,<%s> NOT found in bans database. Play nice now!>",
                               player_name, ipaddr, steam_id);
        }
    }
    ...
    else
    {
        do // In case there is more than 1 row (perhaps 1 by steam_id & 1 by ip)
        {
            ...
            if (ban_length <= 0 || ban_remaining > 0.0)
            {
                ...
                LogAction(client, -1, "[MySQL Bans] Player <%s>,<%s>,<%s> is BANNED for %s",
                          player_name, ipaddr, steam_id, ban_reason);

                /* Error Checking */
                if (check >= 1)
                {
                    PrintCenterTextAll("[MySQL Bans] Player <%s>,<%s>,<%s> is BANNED for %s",
                                       player_name, ipaddr, steam_id, ban_reason);

                    PrintToServer("[MySQL Bans] Player <%s>,<%s>,<%s> is BANNED for %s",
                                  player_name, ipaddr, steam_id, ban_reason);

                    if (check >= 2)
                    {
                        PrintToChatAll("[MySQL Bans] Player <%s>,<%s>,<%s> is BANNED for %s",
                                       player_name, ipaddr, steam_id, ban_reason);
                    }
                }
            ...
            else
            {
                ...
                if (check >= 1)
                {
                    LogAction(client, -1, "[MySQL Bans] <%s>,<%s>,<%s> ban expired. Play nice now!>",
                              player_name, ipaddr, steam_id);

                    PrintToServer("\x04[MySQL Bans]\x03 <%s>,<%s>,<%s> ban expired. Play nice now!>",
                                  player_name, ipaddr, steam_id);

                    if (check >= 2)
                    {
                        PrintToChatAll("\x04[MySQL Bans]\x03 <%s>,<%s>,<%s> ban expired. Play nice now!>",
                                       player_name, ipaddr, steam_id);
                    }
                }
            }
...
KickBannedClient(client, ban_length, Float:ban_remaining, const String:ban_reason[], const String:authid[])
{
    ...
    if (IsClientInGame(client))
    {
        LogAction(client, -1, "[MySQL Bans] Player <%N>,<%s> has been kicked and banned", client, authid);

        /* Error Checking */
        new check = GetConVarInt(ErrorChecking);
        if (check >= 1)
        {
            PrintCenterTextAll("[MySQL Bans] Player <%N>,<%s> has been kicked and banned", client, authid);
            PrintToServer("[MySQL Bans] Player <%N>,<%s> has been kicked and banned", client, authid);

            if (check >= 2)
                PrintToChatAll("[MySQL Bans] Player <%N>,<%s> has been kicked and banned", client, authid);
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            Format(message, sizeof(message),"You have been Banned for %s\nyou have %2.1f weeks left",
                                   ban_reason, ban_remaining);
                        }
                        else
                        {
                            ban_remaining /= 4.0;
                            Format(message, sizeof(message),"You have been Banned for %s\nyou have %2.1f months left",
                                   ban_reason, ban_remaining);
                        }
                    }
                }
    ...
    {
        LogAction(0, -1, "[MySQL Bans] Player <%d>,<%s> has been kicked and banned", client, authid);

        /* Error Checking */
        new check = GetConVarInt(ErrorChecking);
        if (check >= 1)
        {
            PrintCenterTextAll("[MySQL Bans] Player <%d>,<%s> has been kicked and banned", client, authid);
            PrintToServer("[MySQL Bans] Player <%d>,<%s> has been kicked and banned", client, authid);

            if (check >= 2)
                PrintToChatAll("[MySQL Bans] Player <%d>,<%s> has been kicked and banned", client, authid);
        }
...
public Action:Command_AddBan(client, args)
{
    ...
    // My log message
    LogAction(client, -1, "[MySQL Bans] \"%L\" added ban (minutes \"%d\") (id \"%s\") (reason \"%s\")",
              client, minutes, authid, reason);

    new check = GetConVarInt(ErrorChecking);
    if (check > 0)
    {
        // Print commands
        PrintCenterTextAll("[MySQL Bans] [STARTING TO] \"%L\" added ban (minutes \"%d\") (id \"%s\") (reason \"%s\")",
                           client, minutes, authid, reason);

        PrintToServer("[MySQL Bans] [STARTING TO] \"%L\" added ban (minutes \"%d\") (id \"%s\") (reason \"%s\")",
                      client, minutes, authid, reason);

        //PrintToChatAll("[MySQL Bans] [STARTING TO] \"%L\" added ban (minutes \"%d\") (id \"%s\") (reason \"%s\")",
        //               client, minutes, authid, reason);
    }
    ...
    LogAction(client, -1, "[MySQL Bans] Auth %s, Ban time %s, Reason %s, Client %s put into database",
              authid, minutes, reason, client);

    /* Error Checking */
    if (check >= 1)
    {
        PrintCenterTextAll("[MySQL Bans] [Putting into DB] Auth %s, Ban time %s, Reason %s, Client %s",
                           authid, minutes, reason, client);

        PrintToServer("[MySQL Bans] [Putting into DB] Auth %s, Ban time %s, Reason %s, Client %s",
                      authid, minutes, reason, client);
    }
...
InsertBan(client, time, flags, const String:reason[], any:source, const String:identity[])
{
    ...
    {
        decl String:bannedBy[64];

        new AdminId:adminId = GetUserAdmin(source);
        if (adminId != INVALID_ADMIN_ID)
            GetAdminUsername(adminId, bannedBy, sizeof(bannedBy));
        else
            bannedBy[0] = '\0';

        if (bannedBy[0] == '\0')
            GetClientName(source,bannedBy,sizeof(bannedBy));

        if (bannedBy[0] != '\0')
        {
            decl String:escName[sizeof(bannedBy)*2+1];
            if (SQL_EscapeString(db, bannedBy, escName, sizeof(escName)))
            {
                StrCat(columns, sizeof(columns),", banned_by");
                Format(values, sizeof(values), "%s, '%s'", values, escName);
            }
            else                    
                LogError("Unable to escape banned_by %s!", bannedBy);
        }
...
public SQL_DeleteBan(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    if (hndl == INVALID_HANDLE || error[0] != '\0')
    {
        LogError("[MySQL Bans] Player NOT Found. ID: '%s', Lookup Failed", identity);
